...
int main() {
    ...
    int n;
    int** jogos = jogos_possiveis(&n);
    int minimos[M], minimo_para_gene[M];
    for (i = 0; i < M; i++) minimo_para_gene[i] = minimos[i] = -1;
    bool freq[N];
    ...
    for (i = 0; i < M; i++) {
        minimos[i] = calc_min(jogos[i]);
        tam_mapa = max(minimos[i], tam_mapa);
        freq[minimos[i]] = true;
    }
    tam_mapa++;  // Tem que incluir o maior também
    for (i = 0; i < N; i++) tam_cromossomo += freq[i];

    int acc = 0;
    for (i = 0; i < M; i++) {
        if (freq[i]) {
            minimo_para_gene[i] = acc++;
        }
    }
    ...
    for (i = 0; i < M; i++) {
        mapa[minimos[i]] = minimo_para_gene[minimos[i]];
        printf("%d -> %d\n", minimos[i], minimo_para_gene[minimos[i]]);
    }
